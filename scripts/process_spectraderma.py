...
#   python process_spectraderma.py --csv "yale_study_protocol_data_bottommiddle_firstright_firstleft.csv" --denoise
# Options:
...
def main():
    ...
    def norm_gulp(x):
        t = str(x).strip().lower()
        if "gulp" not in t: return ""
        m = re.search(r"gulp\s*([123])", t)
        return f"gulp {m.group(1)}" if m else "gulp"
    ...
    if args.denoise:
        ...
        df[channel_cols] = X
        df.to_csv(outdir / "denoised_signals.csv", index=False)
    ...
    metrics.to_csv(outdir / "metrics_pct.csv", index=False)
    ...
    amp_tbl.to_csv(outdir / "pivot_mean_amplitude_pct_by_loc_gulp.csv")
    auc_tbl.to_csv(outdir / "pivot_mean_auc_pct_by_loc_gulp.csv")
    ...
    for loc in locations:
        ...
        if any_plot:
            ...
            plt.savefig(outdir / f"{loc.replace(' ','_')}_gulps_overlay_norm.png", dpi=150); plt.close()
        ...
        for g in gulps:
            ...
            if any_plot:
                ...
                plt.legend(); plt.tight_layout()
                plt.savefig(outdir / f"{g.replace(' ','_')}_locations_overlay_norm.png", dpi=150); plt.close()
    ...
    if args.plot_channels:
        ...
        for loc in locations:
            for g in gulps:
                ...
                fig.tight_layout()
                fig.savefig(chan_dir / f"{slug(loc)}_{slug(g)}_channels_{suffix}.png", dpi=150)
                plt.close(fig)

    print("Done. Outputs in:", outdir.as_posix())
